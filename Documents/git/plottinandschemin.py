# -*- coding: utf-8 -*-
"""
Created on Sat Dec 14 01:24:10 2019
...
ax.plot(x, hypothesis(x, theta1[0]), color=colors[0], ls='--', lw=1,
           label=r'$\theta_1 = {:.3f}$'.format(theta1[0]))
for j in range(1,N):
    ax.annotate('', xy=(theta1[j], J[j]), xytext=(theta1[j-1], J[j-1]),
                   arrowprops={'arrowstyle': '->', 'color': 'r', 'lw': 1},
                   va='center', ha='center')
    tangent_line(f1,theta1[N-j],-1,1)
    ax.plot(x, hypothesis(x, theta1[j]), color=colors[j], lw=2,
