...
# %% Loading data ############################################################
data         = pd.read_csv('./Data/letsdodis2i.csv')
...
vixret       = pd.DataFrame(vixret.pct_change().dropna().values)
...
NCPNL        = NCPNL.values[1:] - NCPNL.values[:-1]  # Change in NCPNL
CPNL         = clong/cshort # Commercial Pressure Net Long
CPNL         = CPNL.values[1:] - CPNL.values[:-1]  # Change in CPNL
TOTPNL       = totreplong/totrepshort #TotRep PNL
TOTPNL       = TOTPNL.values[1:] - TOTPNL.values[:-1] # Change in TotRep PNL
NONPNL       = nonreplong/nonrepshort # Non-Reportable Pressure Net Long
NONPNL       = NONPNL.values[1:] - NONPNL.values[:-1] # Change in NCPNL
...
for i in dfall.columns:
    ...
    dfall[i]    = np.where(abs(dfall[i])>=4*np.std(dfall[i]), np.nan, dfall[i])
    dfall       = dfall.ffill()
...
## Export tables to latex
with open('params1copy.tex','w') as tf:
    tf.write(params1copy.to_latex(float_format='%.4f'))
""" 
##############################################################################
##############################################################################    
# %% OLS REGRESSIONS ##########################################################

######  Define regdata - RegressionDataset ################################### 
...
for resp in list_of_responses:
    for exog in ['cefd','NONPNL', 'CPNL', 'NCPNL', 'OI', 'vixret']:
        ...
        resid1.pop()
        resid1.append(reg1[len(reg1)-1].resid)
        white1gls.append(sm.stats.diagnostic.het_white(resid1[len(reg1)-1],
                                                  sm.add_constant(regdata[[exog]])))
        acorr1gls.append(sm.stats.diagnostic.acorr_breusch_godfrey(reg1[len(reg1)-1]))
...
for resp in list_of_responses:
    ...
    sigma       = np.array(rho**order,dtype=np.float32)
    reg2.pop()
    reg2.append(sm.GLS.from_formula(formula, data=regdata, sigma=sigma).fit())
    resid2.pop()
    resid2.append(reg2[len(reg2)-1].resid)
    white2gls.append(sm.stats.diagnostic.het_white(resid2[len(reg2)-1], 
                                                     sm.add_constant(regdata[['NONPNL','CPNL','NCPNL', 'OI']])))
    acorr2gls.append(sm.stats.diagnostic.acorr_breusch_godfrey(reg2[len(reg2)-1]))
    tvals2.append(reg2[len(reg2)-1].tvalues)
    print(reg2[len(reg2)-1].summary())
    
tvals2copy = pd.DataFrame(tvals2.copy())
print('Mean of pvalues =', np.abs(tvals2copy).mean(axis=0))
...
for train_index, test_index in tsplit.split(regdata.index):
    ...
    nbyn              = np.arange(len(train_index),dtype=np.int32)
    order             = np.array(toeplitz(nbyn),dtype=np.int32) 
    ...
    for resp in list_of_responses:
        ...
        sigma       = np.array(rho**order,dtype=np.float32)
        reg3.pop()
        reg3.append(sm.GLS.from_formula(formula, data=regdatatest, sigma=sigma).fit())
        resid3.pop()
        resid3.append(reg3[len(reg3)-1].resid)
        bp33.append(sm.stats.diagnostic.het_white(resid3[len(reg3)-1], 
                                                         sm.add_constant(regdatatest)))
        bG33.append(sm.stats.diagnostic.acorr_breusch_godfrey(reg3[len(reg3)-1]))
        fvals3.append(reg3[len(reg3)-1].fvalue)
        rsquared3.append(reg3[len(reg3)-1].rsquared)
        print(reg3[len(reg3)-1].summary())
    
fvals3copy = pd.DataFrame(fvals3.copy())
rsquared3copy = pd.DataFrame(rsquared3.copy())
print('Mean of pvalues =', np.abs(fvals3copy).mean(axis=0))
...
for resp in list_of_responses:
    for exog in notff3:
        var1.append(VAR(regdata[[resp,exog]],dates=regdata.index, freq='M').fit(maxlags=12, ic='aic'))
        varesults1.append(var1[len(var1)-1].test_inst_causality(causing=exog, signif=0.1))
        varesults11.append(var1[len(var1)-1].test_inst_causality(causing=resp, signif=0.1))
...
### Plot results
plt.hist(var1yesno,3)
plt.hist(var11yesno,3)
...
regdata2          = pd.concat([retpos,dfallPC],axis=1,ignore_index=False)
...
for i in range(trainmin,trainmax):
    train, test   = regdata[notff3].iloc[:i], regdata[notff3].iloc[i:i+5]
...
for i in range(0,y_predLin.shape[1]-1,2):
    linresults.append(r2_score(y_predLin[:,i],y_predLin[:,i+1]))
    axLin.append(np.array([y_predLin[:,i],y_predLin[:,i+1]]).T)
    linresid.append(axLin[len(axLin)-1].T[0]-axLin[len(axLin)-1].T[1]) # Heteroscedasticity? 
    logresults.append(accuracy_score(y_predLog[:,i],y_predLog[:,i+1]))
    axLog.append(np.array([y_predLog[:,i],y_predLog[:,i+1]]).T)
...
for i in range(0,y_predLin2.shape[1]-1,2):
    linresults2.append(r2_score(np.roll(y_predLin2[:,i],1,axis=0),y_predLin2[:,i+1]))
    axLin2.append(np.array([np.roll(y_predLin2[:,i],1,axis=0),y_predLin2[:,i+1]]).T)
    logresults2.append(accuracy_score(y_predLog2[:,i],y_predLog2[:,i+1]))
    axLog2.append(np.array([y_predLog2[:,i],y_predLog2[:,i+1]]).T)
...
for i in range(0,y_predLinridge.shape[1]-1,2):
    ridgeresults.append(r2_score(np.roll(y_predLinridge[:,i],1,axis=0),y_predLinridge[:,i+1]))
    axLinridge.append(np.array([np.roll(y_predLinridge[:,i],1,axis=0),y_predLinridge[:,i+1]]).T)
    lassoresults.append(r2_score(np.roll(y_predLinlasso[:,i],1,axis=0),y_predLinlasso[:,i+1]))
    axLinlasso.append(np.array([np.roll(y_predLinlasso[:,i],1,axis=0),y_predLinlasso[:,i+1]]).T)
...
model           = {'RFR': GridSearchCV(RFR, param_grid=random_grid, scoring='r2',
                                       ...
                                       iid=True, n_jobs=-1).fit(X_trainS, y_train2[resp]).best_estimator_,
                   'RFC': GridSearchCV(RFC, param_grid=random_grid, scoring='accuracy',
                                        return_train_score=True, cv=tsplit.split(X_trainS.index),
                                        iid=True, n_jobs=-1).fit(X_trainS, y_train1[resp]).best_estimator_}
...
plt.barh(range(n_features), forest_reg[-1].feature_importances_, align='center')
...
i_tree = 0
for tree_in_forest in forest_reg.estimators_:
