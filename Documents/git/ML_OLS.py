...
###### Model training and testing ############################################
    for resp in list_of_responses:
        for exog in notff3:
            ...
            ## Predictions
            lin_regWO.append(model['OLSwof'].fit(Xwof_train_L, \
                                                  y_train2[resp].iloc[y_train2.index.isin(Xwof_train_L.index)]))
            lin_regW.append(model['OLSwf'].fit(Xwf_train_L, \
...
#             ## Performances - R squared - Inaccurate
...
# =============================================================================
""" PLOT RESULTS BEFORE RUNNING NEXT PIECE OF CODE """
# =============================================================================
### Structuring and dataframing results
linresultsWO = pd.DataFrame(np.split(np.array(linresultsWO), 6), 
...
linresultsW  = pd.DataFrame(np.split(np.array(linresultsW), 6), 
                            columns=list_of_responses*5, 
...
##############################################################################
    for resp in list_of_responses:
        for pc in pclist:
            ...
            model      = {'OLSwf': LinearRegression(fit_intercept=False, normalize=False).fit(Xwf_train_L, \
                                                                                              y_train2[resp].iloc[y_train2.index.isin(Xwf_train_L.index)]),}
            ## Predictions
            lin_regWPCA.append(model['OLSwf'].fit(Xwf_train_L, \
                                                  y_train2[resp].iloc[y_train2.index.isin(Xwf_train_L.index)]))
            lin_regcoefWPCA.append(lin_regWPCA[-1].coef_)
            y_predLinWPCA.append(lin_regWPCA[len(lin_regWPCA)-1].predict(Xwf_test_L))
            axLinWPCA.append(np.array([y_test2[resp].iloc[y_test2.index.isin(Xwf_test_L.index)], \
                                       y_predLinWPCA[len(lin_regWPCA)-1]]).T)
            ## Heteroscedasticity and autocorrelation tests
...
linresultsWPCA  = pd.DataFrame(np.split(np.array(linresultsWPCA), 5), columns=list_of_responses*5, index=pclist)
print('Mean of R2 score with feature = ',
