...
### Plot results
plt.figure()
plt.bar(range(0,len(var1pvals)),var1pvals, alpha=.5, label='With feature', color='r', align='edge')
plt.bar(range(0,len(var11pvals)), var11pvals, alpha=.5, label= 'Without feature', color='b', align='edge')
...
#### VAR 2
# Using lagged values of responses and features' principal components (individual)
var2        = []
varesults2  = []
varesults22 = []
var2_acorr  = []
...
for resp in list_of_responses:
    for exog in notff3:
        var2.append(VAR(regdataPC[[resp, exog]],dates=regdata.index).fit(maxlags=None, ic='aic', trend='c'))
        varesults2.append(var2[len(var2)-1].test_causality(caused=resp, causing=[exog], kind='f', signif=0.1))
        varesults22.append(var2[len(var2)-1].test_causality(caused=resp,causing=[resp], kind='f', signif=0.1))
        var2aic.append(var2[len(var2)-1].aic)
        var2pvals.append(varesults2[len(var2)-1].pvalue)
        var22pvals.append(varesults22[len(var2)-1].pvalue)
        var2yesno.append(varesults1[len(var2)-1].conclusion)
        var22yesno.append(varesults11[len(var2)-1].conclusion)
...
retpos            = Pdata3.abs() > pd.core.window.Expanding(Pdata3, min_periods=20, axis=0).std().bfill()
...
sns.regplot(regdata2[['vixret']],regdata2[[resp]], logistic=1)
...
dflag             = pd.concat([pd.DataFrame(np.roll(dfall,1,axis=0),  # First lag
                                            index=dfall.index, 
...
tsplit          = TimeSeriesSplit(n_splits=15, max_train_size=270)
...
######## Data Splitting and Scaling ##########################################
    ...
    Xwf_train, Xwf_test   = regdata[responsesL + ff3 + notff3].iloc[i-trainmin:i], regdata[responsesL + ff3 + notff3].iloc[i:i+1]
    #
    ...
    # Step2: Standardizing training sets
    ...
    # Transform train AND test sets WITHOUT FEATURE LAGS
    ...
    # Transform train AND test sets WITH FEATURE LAGS
...
##############################################################################
    for resp, respL in zip(list_of_responses, responsesL):
        ...
        model      = {'OLSwof': LinearRegression(fit_intercept=False, normalize=False).fit(Xwof_train[respL],y_train2[resp]),
                      'OLSwf': LinearRegression(fit_intercept=False, normalize=False).fit(Xwf_train[[respL] + ff3 + notff3],y_train2[resp]),}
        ## Predictions
        ...
        y_predLinW.append(lin_regW[len(lin_regW)-1].predict(Xwf_test[[respL] + ff3 + notff3]))
        axLinWO.append(np.array([y_test2[resp], y_predLinWO[len(lin_regWO)-1]]).T)
        axLinW.append(np.array([y_test2[resp], y_predLinW[len(lin_regW)-1]]).T)
        ## Performances
        ## Compare train set performance
        #trainscoreWO.append(model['OLSwof'].score(Xwof_train[respL], y_train2[resp]))
        #trainscoreW.append(model['OLSwf'].score(Xwf_train[[respL] + ff3 + notff3], y_train2[resp]))
        ## Compare test set performance
        #testscoreWO.append(model['OLSwof'].score(Xwof_test[respL][:-1], y_test2[resp][1:]))
        #testscoreW.append(model['OLSwf'].score(Xwf_test[[respL] + ff3 + notff3][:-1], y_test2[resp][1:]))
t1 = timer()
print(t1-t0)
...
## Plot test results, LinearRegression
plt.bar(np.arange(0,len(testscoreWO),step=1), height=testscoreWO, 
                  align='edge', alpha=0.5, label='Without Features')
plt.bar(np.arange(0,len(testscoreW),step=1), height=testscoreW, 
                  align='edge', alpha=0.5, label='With Features')
plt.xticks(ticks=np.arange(0,len(testscoreWO),step=6))
plt.legend()
plt.grid(b=None,axis='x')
...
y_predLinW0  = np.array(axLinWO).squeeze().reshape(trainmax-trainmin,12)
y_predLinW   = np.array(axLinW).squeeze().reshape(trainmax-trainmin,12)
linresultsWO = []
axLinWO      = []
linresid     = []
linresultsW  = []
...
for i in range(0,y_predLinW0.shape[1]-1,2):
    ...
    axLinWO.append(np.array([y_predLinW0[1:,i],y_predLinW0[:-1,i+1]]).T)
    ...
    axLinW.append(np.array([y_predLinW[1:,i],y_predLinW[:-1,i+1]]).T)
"""
...
######## Apply PCA transformation ############################################
    ...
    # Dataframe train set
    Xwf_trainPCA    = pd.DataFrame(Xwf_trainPCA, index=Xwf_train.index, columns=['PC'+ str(i+1) for i in range(Xwf_trainPCA.shape[1])])
    # Dataframe test set
...
##############################################################################
    for resp, respL in zip(list_of_responses, responsesL):
        # Concatenate the respective y_t-1, y_t-2 with all PCs
        Xwf_trainPCA = pd.concat([Xwof_train[respL],Xwf_trainPCA], axis=1)
        Xwf_testPCA  = pd.concat([Xwof_test[respL],Xwf_testPCA], axis=1)
        ## Fit regression
        model      = {'OLSwf': LinearRegression(fit_intercept=False, normalize=False).fit(Xwf_trainPCA, y_train2[resp]),}
        ## Predictions
        lin_regWPCA.append(model['OLSwf'])
        y_predLinWPCA.append(lin_regWPCA[len(lin_regWPCA)-1].predict(Xwf_testPCA))
        axLinWPCA.append(np.array([y_test2[resp], y_predLinWPCA[len(lin_regWPCA)-1]]).T)
        ## Performance
        ## Compare train set performance
        #trainscoreWOPCA.append(model['OLSwof'].score(Xwof_train[respL], y_train2[resp]))
        #trainscoreWPCA.append(model['OLSwf'].score(Xwf_trainPCA, y_train2[resp]))
        ## Compare test set performance
        #testscoreWOPCA.append(model['OLSwof'].score(Xwof_test[respL][:-1], y_test2[resp][1:]))
        #testscoreWPCA.append(model['OLSwf'].score(Xwf_testPCA[:-1], y_test2[resp][1:]))
t1 = timer()
print(t1-t0)

## Plot train results, LinearRegression
plt.bar(np.arange(0,len(trainscoreWOPCA),step=1), height=trainscoreWOPCA, 
                  align='edge', alpha=0.5, label='Without PCs')
plt.bar(np.arange(0,len(trainscoreWPCA),step=1), height=trainscoreWPCA, 
                  align='edge', alpha=0.5, label='With PCs')
plt.xticks(ticks=np.arange(0,len(trainscoreWOPCA),step=6))
plt.legend()
plt.grid(b=None,axis='x')

## Plot test results, LinearRegression
plt.bar(np.arange(0,len(testscoreWOPCA),step=1), height=testscoreWOPCA, 
                  align='edge', alpha=0.5, label='Without PCs')
plt.bar(np.arange(0,len(testscoreWPCA),step=1), height=testscoreWPCA, 
                  align='edge', alpha=0.5, label='With PCs')
plt.xticks(ticks=np.arange(0,len(testscoreWOPCA),step=6))
plt.legend()
plt.grid(b=None,axis='x')
...
# Restructure predictions so that all of them are in one matrix. 
# Each columns alternates between true y_test and predited y_test
y_predLinWPCA   = np.array(axLinWPCA).squeeze().reshape(trainmax-trainmin,12)
linresultsWPCA  = []
axLinWPCA       = []
# Get the results
for i in range(0,y_predLinWPCA.shape[1]-1,2):
    linresultsWPCA.append(mean_squared_error(y_predLinWPCA[1:,i],y_predLinWPCA[:-1,i+1]))
    axLinWPCA.append(np.array([y_predLinWPCA[1:,i],y_predLinWPCA[:-1,i+1]]).T)
"""
...
testscoreLW      = []
#
lasso_params    = {'alpha':[0.005, 0.01, 0.02, 0.05, 0.1]}
ridge_params    = {'alpha':[5,10,20,50, 100],
...
##############################################################################
    for resp, respL in zip(list_of_responses, responsesL):
        model        = {'LassoWO': GridSearchCV(Lasso(fit_intercept=True, normalize=False, random_state=42,
                        ...
                        'LassoW':  GridSearchCV(Lasso(fit_intercept=True, normalize=False, random_state=42,
                                                      selection='random', max_iter=1000,
                                                      tol=0.001), 
                                      param_grid=lasso_params, 
                                      scoring='neg_mean_squared_error', 
                                      return_train_score=True, 
                                      cv=tsplit.split(Xwof_train.index), iid=True).fit(Xwf_train[[respL]+ ff3 + notff3], y_train2[resp]).best_estimator_,
                        'RidgeWO': GridSearchCV(Ridge(fit_intercept=True, normalize=False, random_state=42, 
                                                      max_iter=1000, tol=0.001), 
                                      param_grid=ridge_params, 
                                      scoring='neg_mean_squared_error', 
                                      return_train_score=True, 
                                      cv=tsplit.split(Xwof_train.index), iid=True).fit(Xwof_train[respL], y_train2[resp]).best_estimator_,
                        'RidgeW':  GridSearchCV(Ridge(fit_intercept=True, normalize=False, random_state=42, 
                                                      max_iter=1000, tol=0.001), 
                                      param_grid=ridge_params, 
                                      scoring='neg_mean_squared_error', 
                                      return_train_score=True, 
        ...
        y_predLinridgeW.append(ridge_regW[len(ridge_regW)-1].predict(Xwf_test[[respL] + ff3 + notff3]))
        ...
        y_predLinlassoW.append(lasso_regW[len(lasso_regW)-1].predict(Xwf_test[[respL]+ ff3 + notff3]))
        ...
        #trainscoreRW.append(model['RidgeW'].score(Xwf_train[[respL] + ff3 + notff3], y_train2[resp]))
        #trainscoreLW.append(model['LassoW'].score(Xwf_train[[respL] + ff3 + notff3], y_train2[resp]))
        ## Compare test set performance
        #testscoreRWO.append(model['RidgeWO'].score(Xwof_test[respL][:-1], y_test2[resp][1:]))
        #testscoreLWO.append(model['LassoWO'].score(Xwof_test[respL][:-1], y_test2[resp][1:]))
        #testscoreRW.append(model['RidgeW'].score(Xwf_test[[respL] + ff3 + notff3][:-1], y_test2[resp][1:]))
        #testscoreLW.append(model['LassoW'].score(Xwf_test[[respL] + ff3 + notff3][:-1], y_test2[resp][1:]))
t1 = timer()
print(t1-t0)

# Restructure predictions so that all of them are in one matrix. 
# Each columns alternates between true y_test and predicted y_test
y_predLinridgeWO    = np.array(axLinridgeWO).squeeze().reshape(trainmax-trainmin,12)
y_predLinridgeW     = np.array(axLinridgeW).squeeze().reshape(trainmax-trainmin,12)
y_predLinlassoWO    = np.array(axLinlassoWO).squeeze().reshape(trainmax-trainmin,12)
y_predLinlassoW     = np.array(axLinlassoW).squeeze().reshape(trainmax-trainmin,12)
ridgeresultsWO      = []
axLinridgeWO        = []
lassoresultsWO      = []
axLinlassoWO        = []
...
for i in range(0,y_predLinridgeWO.shape[1]-1,2):
    ridgeresultsWO.append(mean_squared_error(y_predLinridgeWO[1:,i],y_predLinridgeWO[:-1,i+1]))
    ridgeresultsW.append(mean_squared_error(y_predLinridgeW[1:,i],y_predLinridgeW[:-1,i+1]))
    lassoresultsWO.append(mean_squared_error(y_predLinlassoWO[1:,i],y_predLinlassoWO[:-1,i+1]))
    lassoresultsW.append(mean_squared_error(y_predLinlassoW[1:,i],y_predLinlassoW[:-1,i+1]))
    axLinridgeWO.append(np.array([y_predLinridgeWO[1:,i],y_predLinridgeWO[:-1,i+1]]).T)
    axLinridgeW.append(np.array([y_predLinridgeW[1:,i],y_predLinridgeW[:-1,i+1]]).T)
    axLinlassoWO.append(np.array([y_predLinlassoWO[1:,i],y_predLinlassoWO[:-1,i+1]]).T)
    axLinlassoW.append(np.array([y_predLinlassoW[1:,i],y_predLinlassoW[:-1,i+1]]).T)
...
## Plot test results, 
plt.bar(np.arange(0,len(testscoreRWO),step=1), height=testscoreRWO, 
                  align='edge', alpha=0.25, label='Ridge without Features', linewidth=1)
plt.bar(np.arange(0,len(trainscoreRW),step=1), height=testscoreRW, 
                  align='edge', alpha=0.25, label='Ridge with Features', linewidth=1)
plt.bar(np.arange(0,len(testscoreLWO),step=1), height=testscoreLWO, 
                  align='edge', alpha=0.25, label=' Lasso without Features', linewidth=1)
plt.bar(np.arange(0,len(testscoreLW),step=1), height=testscoreLW, 
                  align='edge', alpha=0.25, label='Lasso with Features', linewidth=1)
plt.xticks(ticks=np.arange(0,len(testscoreLWO),step=6))
plt.legend()
plt.grid(b=None,axis='x')
"""
...
######## Apply PCA transformation ############################################
    ...
    Xwf_testPCA     = pca.transform(Xwf_test)  # Transformed test sets
    # Dataframe train set
    Xwf_trainPCA    = pd.DataFrame(Xwf_trainPCA, index=Xwf_train.index, 
                                   columns=['PC'+ str(i+1) for i in range(Xwf_trainPCA.shape[1])])
    # Dataframe test set
    Xwf_testPCA     = pd.DataFrame(Xwf_testPCA, index=Xwf_test.index, 
                                   columns=Xwf_trainPCA.columns)
    ####regdataPC.append(X_pca2.values) Ignore
##############################################################################
    for resp, respL in zip(list_of_responses, responsesL):
        # Concatenate the respective y_t-1, y_t-2 with all PCs
        ...
        Xwf_testPCA  = pd.concat([Xwof_test[respL],Xwf_testPCA], axis=1) 
        # Fit regressions
        model        = {'LassoWPCA':  GridSearchCV(Lasso(fit_intercept=True, normalize=False, random_state=42,
                                                      selection='random', max_iter=1000, 
                                                      tol=0.001), 
                                      param_grid=lasso_params, 
                                      scoring='r2', 
                                      return_train_score=True, 
                                      cv=tsplit.split(Xwof_train.index), iid=True).fit(Xwf_trainPCA, y_train2[resp]).best_estimator_,
                        'RidgeWPCA':  GridSearchCV(Ridge(fit_intercept=True, normalize=False, random_state=42, 
                                                      max_iter=1000, tol=0.001), 
                                      param_grid=ridge_params, 
                                      scoring='r2', 
                                      return_train_score=True, 
        ...
        y_predLinridgeWPCA.append(ridge_regWPCA[len(ridge_regWPCA)-1].predict(Xwf_testPCA))
        axLinridgeWPCA.append(np.array([y_test2[resp][1:], y_predLinridgeWPCA[len(ridge_regWPCA)-1][:-1]]).T)
        ## LASSO Predictions
        lasso_regWPCA.append(model['LassoWPCA'])
        y_predLinlassoWPCA.append(lasso_regWPCA[len(lasso_regWPCA)-1].predict(Xwf_testPCA))
        axLinlassoWPCA.append(np.array([y_test2[resp][1:], y_predLinlassoWPCA[len(lasso_regWPCA)-1][:-1]]).T)
        ## Performances
        ## Compare train set performance
        #trainscoreRWPCA.append(model['RidgeWPCA'].score(Xwf_trainPCA, y_train2[resp]))
        #trainscoreLWPCA.append(model['LassoWPCA'].score(Xwf_trainPCA, y_train2[resp]))
        ## Compare test set performance
        #testscoreRWPCA.append(model['RidgeWPCA'].score(Xwf_testPCA[:-1], y_test2[resp][1:]))
        #testscoreLWPCA.append(model['LassoWPCA'].score(Xwf_testPCA[:-1], y_test2[resp][1:]))
t1 = timer()
print(t1-t0)

# Restructure predictions so that all of them are in one matrix. 
# Each columns alternates between true y_test and predicted y_test
y_predLinridgeWPCA  = np.array(axLinridgeWPCA).squeeze().reshape(trainmax-trainmin,12)
y_predLinlassoWPCA  = np.array(axLinlassoWPCA).squeeze().reshape(trainmax-trainmin,12)
ridgeresultsWPCA    = []
axLinridgeWPCA      = []
lassoresultsWPCA    = []
axLinlassoWPCA      = []
# Get the results
for i in range(0,y_predLinridgeWO.shape[1]-1,2):
    ridgeresultsWPCA.append(mean_squared_error(y_predLinridgeW[1:,i],y_predLinridgeW[:-1,i+1]))
    lassoresultsWPCA.append(mean_squared_error(y_predLinlassoW[1:,i],y_predLinlassoW[:-1,i+1]))
    axLinridgeWPCA.append(np.array([y_predLinridgeW[1:,i],y_predLinridgeW[:-1,i+1]]).T)
    axLinlassoWPCA.append(np.array([y_predLinlassoW[1:,i],y_predLinlassoW[:-1,i+1]]).T)
...
plt.bar(np.arange(0,len(testscoreRWO),step=1), height=testscoreRWO, 
                  align='edge', alpha=0.25, label='Ridge without PCs', linewidth=1)
plt.bar(np.arange(0,len(trainscoreRWPCA),step=1), height=testscoreRWPCA, 
                  align='edge', alpha=0.25, label='Ridge with PCs', linewidth=1)
plt.bar(np.arange(0,len(testscoreLWO),step=1), height=testscoreLWO, 
                  align='edge', alpha=0.25, label=' Lasso without PCs', linewidth=1)
plt.bar(np.arange(0,len(testscoreLWPCA),step=1), height=testscoreLWPCA, 
                  align='edge', alpha=0.25, label='Lasso with PCs', linewidth=1)
plt.xticks(ticks=np.arange(0,len(testscoreLWO),step=6))
plt.legend()
plt.grid(b=None,axis='x')
